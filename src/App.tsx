...
const App: React.FC = () => {
	...
	useEffect(() => {
		axios.get('http://localhost:5000/heatmap')
			...
			.catch(error => {
				console.error('There was an error fetching the heatmap data!', error);
			});
	}, []);
									...
									<div>
										...
										<ul>
											{heatmapData.map((data, index) => (
												<li key={index}>{data.location}: {data.density} people</li>
											))}
										</ul>
									</div>
									<div style={{ display: 'flex', justifyContent: 'center' }}>
										<Link to="#" onClick={(e) => {
											e.preventDefault(); window.open('http://localhost:3000/video_feed', '_blank'); 
										}}>
